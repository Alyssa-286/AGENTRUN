"""
run_eval_base.py — Run the benchmark against the base Qwen 2.5 7B model (no LoRA).

Uses the exact same 23 held-out tasks from eval/tasks.py as run_eval_finetuned.py,
but sends them to the Colab-hosted Qwen 2.5 7B-Instruct BASE model (no adapter).

Usage:
    python run_eval_base.py

Requirements:
    - COLAB_SERVER_URL_BASE environment variable set (or pass via --url argument)
...
async def run_evaluation(
    ...
    """Run the benchmark against the base Qwen model."""
    # Get server URL from argument or environment; never hard-code it.
    server_url = colab_server_url or os.environ.get("COLAB_SERVER_URL_BASE")
    if not server_url:
        raise RuntimeError(
            "COLAB_SERVER_URL_BASE not set. Either pass --url argument or set the environment variable."
        )

    print("=" * 60)
    print("Base Qwen 2.5 7B Benchmark")
    print(f"Server: {server_url}")
    print("=" * 60)

    # Initialize remote agent — uses the same QwenRemoteClient from ft_agent.py
...
async def main():
    parser = argparse.ArgumentParser(description="Run benchmark against base Qwen model")
    parser.add_argument(
        "--url",
        dest="server_url",
        help="Colab server URL (or set COLAB_SERVER_URL_BASE env var)",
    )
    parser.add_argument(
        "--max-steps",
        type=int,
    ...
    parser.add_argument(
        "--json-output",
        default="results/eval_results_base.json",
        help="Output JSON file path (default: results/eval_results_base.json)",
    )
    parser.add_argument(
        "--md-output",
        default="results/eval_report_base.md",
        help="Output markdown file path (default: results/eval_report_base.md)",
    )
